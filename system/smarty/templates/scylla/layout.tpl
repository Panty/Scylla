	...
	<body>
		...
		<div id="content">
			<div id="status"> Statut du serveur : <span class="badge badge-success">ON</span>
				Comptes créés : <span class="badge">{$accounts}</span> Personnages créés : <span class="badge">{$characters}</span> Guildes créées : <span class="badge">{$guilds}</span>
			</div>
			<table>
				<tr>
			...
			{/if}
				<li class="nav-header">Accueil</li>
    				...
    				<li><a href="{$URL}index.php/server/cgu">Règlement</a></li>
		...
		<div id="footer">
			<div class="login">
				...
				{else}
                <button class="btn btn-danger dropdown-toggle" onClick="window.location.href='{$URL}index.php/user/logout'">Déconnexion ({$smarty.session.pseudo})</button>
				{/if}
			</div>
			<div class="search">
